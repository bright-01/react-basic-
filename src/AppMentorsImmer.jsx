...
export default function AppMentor() {
    const [person, updateImmer] = useImmer(initialPerson);
    ...
    const personAddHandle = () => {
       ...
       updateImmer(person => person.mentors.push({name, title}));
    }
    const personDeleteHandle = () => {
        const name = prompt(`삭제할 이름은?`);
        updateImmer( person => {
            const mentorIndex = person.mentors.findIndex(m => m.name = name);
            person.mentors.slice(mentorIndex, 1);
        })
